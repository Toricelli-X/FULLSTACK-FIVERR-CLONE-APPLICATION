...
function Gig() {
  ...
  return (
    ...
    { isLoading?"Loading":error?"Something went wrong": <div className="container">
        <div className="left">
          <span className="breadcrumbs">Liverr {">"} Graphics & Design {">"}
          </span>
          <h1>{data.title}</h1>
        { isLoadingUser?"Loading": userError?"Something went wrong": <div className="user">
            ...
            <span>{userData.username}</span>
             {!isNaN(data.totalStars
             /data.starNumber)} &&  <div className="stars">
              {
                Array(Math.round(data.totalStars
                  /data.starNumber).fill().map((item,i)=>{
                     <img src="/img/star.png" alt=""key={i} />
                  }))
              }
              <span>
              {Math.round(data.totalStars
              /data.starNumber)}
              </span>
            </div>
          </div>}
